...
export default function ParentDashboardPage() {
  ...
  useEffect(() => {
    const fetchPortalData = async () => {
      ...
      try {
        // 1. Fetch current student profile details
        ...
        // 2. Fetch ALL report cards plainly
        ...
        // 3. Fetch financial data mapped perfectly to your explicit schema
        const { data: feeRecords } = await supabase
          .from('fees')
          .select('*')
          .eq('student_id', studentId) // Direct strict match on the unique UUID reference key
          .order('payment_date', { ascending: false })

        if (feeRecords && feeRecords.length > 0) {
          setPayments(feeRecords)

          // Calculate sum total paid dynamically from the matching logs
          let paidSum = 0
      ...
      <html>
        ...
        <body>
          <div class="receipt-box">
            ...
            <div class="footer">
              <p>Thank you for your continuous support towards Frankies Edutech excellence.</p>
              <p style="font-size:10px; margin-top:5px;">This is a computer-generated account document. No physical signature required.</p>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen p-4 md:p-8" style={{ background: '#0f172a', fontFamily: 'Poppins, sans-serif' }}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8 pb-4 border-b" style={{ borderColor: '#334155' }}>
          <div>
            <h1 className="text-xl font-bold tracking-tight" style={{ color: '#e2e8f0' }}>
              Frankies Edutech Portal
            </h1>
            <p className="text-xs" style={{ color: '#38bdf8' }}>Learner & Parent Dashboard</p>
          </div>
          <button
        ...
        {student && (
          <div className="rounded-2xl p-6 mb-8 border flex flex-col md:flex-row md:items-center justify-between gap-4" style={{ background: '#1e293b', borderColor: '#334155' }}>
            <div>
              <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded" style={{ background: '#0284c7', color: '#e0f2fe' }}>
                Active Learner
              </span>
              <h2 className="text-xl font-semibold mt-2" style={{ color: '#e2e8f0' }}>
                {student.full_name}
              </h2>
              <p className="text-xs mt-1" style={{ color: '#94a3b8' }}>
                ID Code: <span className="font-mono text-sky-400">{student.learner_code}</span>
              </p>
            </div>
        ...
        {/* TAB 1: ACADEMICS VIEW */}
        {activeTab === 'academics' && (
          <div>
            {reports.length === 0 ? (
              <div className="rounded-xl p-8 text-center border" style={{ background: '#1e293b', borderColor: '#334155', color: '#475569' }}>
                No terminal report cards have been published for this learner yet.
              </div>
            ) : (
                ...
                {reports.map((rc) => (
                  <div key={rc.id} className="rounded-xl p-5 border flex flex-col justify-between gap-4" style={{ background: '#1e293b', borderColor: '#334155' }}>
                    <div>
                      ...
                      <p className="text-[11px] mt-2" style={{ color: '#64748b' }}>Published: {new Date(rc.uploaded_at).toLocaleDateString()}</p>
                    </div>
                    <div className="pt-2">
                      <a href={rc.file_url} target="_blank" rel="noopener noreferrer" className="w-full text-center block py-2 rounded-lg text-xs font-semibold transition" style={{ background: '#38bdf8', color: '#0f172a' }}>
        ...
        {activeTab === 'finance' && (
          <div className="space-y-6">
            ...
            <div className="p-4 rounded-xl border max-w-sm" style={{ background: '#111827', borderColor: '#22c55e40' }}>
              <p className="text-[10px] font-medium uppercase tracking-wider" style={{ color: '#22c55e' }}>Total Fees Paid (Accumulated)</p>
              <p className="text-xl font-bold mt-1" style={{ color: '#e2e8f0' }}>GHS {totalPaid.toFixed(2)}</p>
            </div>

            {/* Financial History List Ledger */}
            <h4 className="text-xs font-medium uppercase tracking-wider" style={{ color: '#64748b' }}>Payment Logs Ledger</h4>
            
            {payments.length === 0 ? (
              <div className="rounded-xl p-8 text-center border" style={{ background: '#1e293b', borderColor: '#334155', color: '#475569' }}>
                No payment transactions have been logged for this learner yet.
              </div>
            ) : (
                  ...
                  <div key={payment.id} className="p-4 rounded-xl border flex justify-between items-center gap-4" style={{ background: '#1e293b', borderColor: '#334155' }}>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold font-mono px-1.5 py-0.5 rounded text-emerald-400 bg-emerald-950/40">
                          + GHS {Number(payment.amount || 0).toFixed(2)}
                        </span>
                        <p className="text-xs font-semibold" style={{ color: '#e2e8f0' }}>
                          Fees Receipt ({payment.term || 'General Fee'})
                        </p>
                      </div>
                      <p className="text-[11px] mt-1" style={{ color: '#64748b' }}>
                        Date Paid: {new Date(payment.payment_date).toLocaleDateString()} | Academic Year: {payment.academic_year || 'N/A'} | Via: {payment.payment_method || 'Cash'}
                      </p>
                    </div>

                    <button
                      onClick={() => handlePrintReceipt(payment)}
                      className="px-3 py-1.5 rounded-lg text-xs font-medium border transition border-slate-700 text-sky-400 hover:bg-sky-950/20"
                    >
                      🖨️ Print Receipt
                    </button>
                  </div>
